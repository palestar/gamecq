...
// last revised: $Date: 2005/04/25 02:40:46 $, $Revision: 1.1.1.1 $
...
LPTSTR CNTService :: GetLastErrorText( LPTSTR lpszBuf, DWORD dwSize ) {
    ...
    if( !dwRet || (dwSize < dwRet+14) )
        ...
        strcpy_s(lpszBuf, dwSize, lpszTemp);
    }
...
void CNTService :: RegisterApplicationLog( LPCTSTR lpszFileName, DWORD dwTypes ) {
	TCHAR szKey[256];
	strcpy_s(szKey, sizeof(szKey), gszAppRegKey);
	strcat_s(szKey, sizeof(szKey), m_lpServiceName);
	HKEY hKey = 0;
	LONG lRet = ERROR_SUCCESS;
 		...
 		if( lRet == ERROR_SUCCESS ) {
			...
			if( lRet == ERROR_SUCCESS ) {
				...
				if( ! * p ) {
					...
					// too.
					strcpy_s(p, dwNewSize, m_lpServiceName);
