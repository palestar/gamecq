        ...
        case WM_CANCELMODE:
			{
				...
				m_parent->SendMessage(msgNew.message, msgNew.wParam, msgNew.lParam);
...
void TFXDataTip::OnPaint() 
{
	...
	for(int i=0;i<tipLength;i++)
	{
		char c = (char)m_tip[i];
		if ( c == '\n' )
			flushLine = true;			// newline
		else if ( c != '\r' )			// ignore /r characters
			line += c;					// normal character

		if ( dc.GetTextExtent( line ).cx > MAX_TIP_WIDTH )
			overFlow = flushLine = true;
		if ( i == (tipLength - 1 ) )
			flushLine = true;			// last character
		...
		if ( flushLine )
		{
		    ...
		    dc.TextOut(lineX, lineY, line);
			lineY += dc.GetTextExtent( line ).cy;
