...
void CButtonsList::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	...
	CBCGToolbarComboBoxButton btnDummy;
	rectClient.InflateRect (1, 1);
	CBCGVisualManager::GetInstance ()->OnDrawComboBorder (pDC, rectClient,
										!IsWindowEnabled (),
										FALSE,
										TRUE,
										&btnDummy);
...
void CButtonsList::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	case VK_UP:
		if (m_pSelButton != NULL)
		{
			...
			if (pos != NULL)
			{
				...
				if (pos != NULL)
				{
					SelectButton ((CBCGToolbarButton*) m_Buttons.GetAt (pos));
				}
			}
		}
		else if (!m_Buttons.IsEmpty ())
		{
			SelectButton ((CBCGToolbarButton*) m_Buttons.GetHead ());
		}
	...
	case VK_END:
		if (!m_Buttons.IsEmpty ())
		{
			SelectButton ((CBCGToolbarButton*) m_Buttons.GetTail ());
		}
		return;
	}			
