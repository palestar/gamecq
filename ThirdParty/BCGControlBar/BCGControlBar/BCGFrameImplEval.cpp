...
{
	if (g_pWorkspace != NULL &&
		cs.hInstance != NULL)	// By Sven Ritter
	{
...
void CBCGFrameImpl::OnLoadFrame()
{
	...
	static const CString strEvaluation = _T("This application uses evluation version of BCGControlBar Library\r\n\r\nPlease visit http://www.bcgsoft.com");
	static const CString strEvaluationCaption = _T("BCGControlBar Library");
	::MessageBox (NULL, strEvaluation, strEvaluationCaption, MB_OK | MB_SETFOREGROUND | MB_ICONINFORMATION);
}
//**************************************************************************************
...
BOOL CBCGFrameImpl::ProcessKeyboard (int nKey)
{
	...
	if (!CBCGToolBar::IsCustomizeMode () &&
		(::GetAsyncKeyState (VK_MENU) & 0x8000) &&	// ALT is pressed by itself
		!(::GetAsyncKeyState (VK_SHIFT) & 0x8000) &&	///--------
		!(::GetAsyncKeyState (VK_CONTROL) & 0x8000))	// By Rodger Bernstein
	{													//---------
		if (OnMenuChar (nKey))
		{
			return TRUE;
...
// More safe version of CFrameWnd::SetDockState - by Benjamin Mayrargue
{
	ASSERT_VALID (m_pFrame);
