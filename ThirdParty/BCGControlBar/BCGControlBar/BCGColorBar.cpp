...
protected:
	...
	virtual BOOL OnToolHitTest(const CWnd* /*pWnd*/, TOOLINFO* pTI)
	{
		...
		if (!CBCGToolBar::GetShowTooltips ())
		{
			return FALSE;
		}
		...
		{
			if (!CBCGColorBar::m_ColorNames.Lookup (m_Color, str))
			{
	...
	virtual void OnChangeParentWnd (CWnd* pWndParent)
	{
		...
		m_pParentBar = DYNAMIC_DOWNCAST (CBCGColorBar, pWndParent);
	}
								...
								BOOL bDrawBorder, BOOL /*bGrayDisabledButtons*/)
{
	...
	if (!bDisabled && !m_bIsLabel && HaveHotBorder () && bDrawBorder && !bCustomizeMode)
	{
		if (m_nStyle & (TBBS_PRESSED | TBBS_CHECKED))
		{
			...
			//-----------------------
			CBCGVisualManager::GetInstance ()->OnDrawButtonBorder (
				pDC, this, rect, CBCGVisualManager::ButtonsIsPressed);
		}
		else if (bHighlight &&
			!(m_nStyle & (TBBS_CHECKED | TBBS_INDETERMINATE)))
		{
			CBCGVisualManager::GetInstance ()->OnDrawButtonBorder (
				pDC, this, rect, CBCGVisualManager::ButtonsIsHighlighted);
		}
	}
...
IMPLEMENT_SERIAL(CBCGColorBar, CBCGPopupMenuBar, 1)

CMap<COLORREF,COLORREF,CString, LPCTSTR> CBCGColorBar::m_ColorNames;

CBCGColorBar::CBCGColorBar()
{
    m_nNumColumns = 0;
	m_nNumRowsHorz = 0;
	m_nNumColumnsVert = 0;
...
//**************************************************************************************
CBCGColorBar::CBCGColorBar (const CArray<COLORREF, COLORREF>& colors,
				COLORREF color, 
...
//**************************************************************************************
CBCGColorBar::CBCGColorBar (CBCGColorBar& src, UINT uiCommandID) :
...
//*************************************************************************************
void CBCGColorBar::AdjustLocations ()
{
...
//***************************************************************************************
CSize CBCGColorBar::CalcSize (BOOL bVertDock)
{
...
//*************************************************************************************
CSize CBCGColorBar::GetColorGridSize (BOOL bVertDock) const
//
...
//**************************************************************************************
int CBCGColorBar::GetExtraHeight (int nNumColumns) const
//
...
BEGIN_MESSAGE_MAP(CBCGColorBar, CBCGPopupMenuBar)
	//{{AFX_MSG_MAP(CBCGColorBar)
	ON_WM_CREATE()
...
/////////////////////////////////////////////////////////////////////////////
// CBCGColorBar message handlers

void CBCGColorBar::SetDocumentColors (LPCTSTR lpszCaption,
						CList<COLORREF,COLORREF>& lstDocColors,
...
//**************************************************************************************
void CBCGColorBar::ContextToSize (BOOL bSquareButtons, BOOL bCenterButtons)
{
...
//**************************************************************************************
void CBCGColorBar::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
	if (m_bIsTearOff)
	{
		CBCGToolBar::OnNcCalcSize(bCalcValidRects, lpncsp);
	}
	else
	{
		CBCGPopupMenuBar::OnNcCalcSize(bCalcValidRects, lpncsp);
	}
}
//**************************************************************************************
void CBCGColorBar::OnNcPaint() 
{
	if (m_bIsTearOff)
	{
		CBCGToolBar::OnNcPaint();
	}
	else
	{
		CBCGPopupMenuBar::OnNcPaint();
	}
}
//**************************************************************************************
int CBCGColorBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
class CBCGColorCCmdUI : public CCmdUI
{
public:
	CBCGColorCCmdUI();
...
CBCGColorCCmdUI::CBCGColorCCmdUI()
{
	m_bEnabled = TRUE;  // assume it is enabled
}
//*************************************************************************************
void CBCGColorCCmdUI::Enable(BOOL bOn)
{
	m_bEnabled = bOn;
	m_bEnableChanged = TRUE;
}
//*************************************************************************************
void CBCGColorBar::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
	ASSERT_VALID (this);

	if (m_nCommandID == 0 || m_nCommandID == (UINT)-1)
	{
		CBCGPopupMenuBar::OnUpdateCmdUI (pTarget, bDisableIfNoHndler);
		return;
	}

	CBCGColorCCmdUI state;
	state.m_pOther = this;
	state.m_nIndexMax = 1;
...
//*************************************************************************************
void CBCGColorBar::DoPaint (CDC* pDC)
{
...
//*************************************************************************************
BOOL CBCGColorBar::OnQueryNewPalette() 
{
    Invalidate();    
	UpdateWindow ();
...
//*************************************************************************************
void CBCGColorBar::OnPaletteChanged(CWnd* pFocusWnd) 
{
...
//*************************************************************************************
BOOL CBCGColorBar::OnSendCommand (const CBCGToolbarButton* pButton)
{
	...
	CBCGColorMenuButton* pColorMenuButton = NULL;

	CBCGPopupMenu* pParentMenu = DYNAMIC_DOWNCAST (CBCGPopupMenu, GetParent ());
	if (pParentMenu != NULL)
	{
		pColorMenuButton = DYNAMIC_DOWNCAST (CBCGColorMenuButton, pParentMenu->GetParentButton ());
	}
	...
	else if (pColorButton->m_bIsOther)
	{
			...
			m_ColorAutomatic : m_ColorSelected, color))
		{
			...
			else if (BCGGetTopLevelFrame (this) != NULL)
			{
	...
	else
	{
		...
		CObList listButtons;
		if (CBCGToolBar::GetCommandButtons (m_nCommandID, listButtons) > 0)
		{
			for (POSITION pos = listButtons.GetHeadPosition (); pos != NULL;)
			{
				CBCGColorMenuButton* pButton = 
		...
		CBCGColorMenuButton::SetColorByCmdID (m_nCommandID, color);
			...
			int			nColDockVert/* = 0*/)
{
	...
	{
		return CBCGPopupMenuBar::Create (pParentWnd, dwStyle, nID);
	}
...
//************************************************************************************
BOOL CBCGColorBar::CreateControl (
				CWnd*			pParentWnd,
				const CRect&	rect,
				UINT			nID,
				int				nColumns,
...
//*************************************************************************************
int CBCGColorBar::InitColors (CPalette* pPalette, CArray<COLORREF, COLORREF>& arColors)
{
...
//*************************************************************************************
void CBCGColorBar::Serialize (CArchive& ar)
{
...
//*************************************************************************************
void CBCGColorBar::ShowCommandMessageString (UINT /*uiCmdId*/)
{
	GetOwner()->SendMessage (WM_SETMESSAGESTRING,
		m_nCommandID == (UINT) -1 ? AFX_IDS_IDLEMESSAGE : (WPARAM) m_nCommandID);
}
//*************************************************************************************
BOOL CBCGColorBar::OpenColorDialog (const COLORREF colorDefault, COLORREF& colorRes)
{
	CBCGColorMenuButton* pColorMenuButton = NULL;
...
//*************************************************************************************
void CBCGColorBar::EnableAutomaticButton (LPCTSTR lpszLabel, COLORREF colorAutomatic, BOOL bEnable)
{
	m_ColorAutomatic = colorAutomatic;
...
//*************************************************************************************
void CBCGColorBar::EnableOtherButton (LPCTSTR lpszLabel, BOOL bAltColorDlg, BOOL bEnable)
{
...
//*************************************************************************************
void CBCGColorBar::SetColor (COLORREF color)
{
...
//***************************************************************************************
void CBCGColorBar::OnMouseMove(UINT nFlags, CPoint point) 
{
...
//***************************************************************************************
void CBCGColorBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if (IsCustomizeMode () && !m_bInternal)
	{
		return;
...
//***************************************************************************************
void CBCGColorBar::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (!IsCustomizeMode () || m_bInternal)
	{
...
//***************************************************************************************
void CBCGColorBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	if (!IsCustomizeMode () || m_bInternal)
	{
...
//***************************************************************************************
BOOL CBCGColorBar::PreTranslateMessage(MSG* pMsg)
{
	...
	return CBCGPopupMenuBar::PreTranslateMessage(pMsg);
}
//*************************************************************************************
void CBCGColorBar::OnDestroy() 
{
	if (m_pParentBtn != NULL)
	{
	...
	CBCGPopupMenuBar::OnDestroy();
}
//****************************************************************************************
BOOL CBCGColorBar::OnKey(UINT nChar)
{
	POSITION posSel = 
		(m_iHighlighted < 0) ? NULL : m_Buttons.FindIndex (m_iHighlighted);
	CBCGToolbarButton* pSelButton = 
	...
	return CBCGPopupMenuBar::OnKey (nChar);
}
//****************************************************************************************
afx_msg LRESULT CBCGColorBar::OnMouseLeave(WPARAM wp,LPARAM lp)
{
	if (m_pParentBtn == NULL && m_pWndPropList == NULL)
	{
		return CBCGToolBar::OnMouseLeave (wp, lp);
	}
...
//**************************************************************************************
BOOL CBCGColorBar::CreatePalette (const CArray<COLORREF, COLORREF>& arColors,
								CPalette& palette)
{
...
//**************************************************************************************
CPalette* CBCGColorBar::SelectPalette (CDC* pDC)
{
	ASSERT_VALID (pDC);
...
//****************************************************************************************
void CBCGColorBar::SetVertMargin (int nVertMargin)
{
	ASSERT_VALID (this);
...
//*****************************************************************************************
void CBCGColorBar::SetHorzMargin (int nHorzMargin)
{
	ASSERT_VALID (this);
