...
//********************************************************************************
// BCGMaskEdit.cpp : implementation file
//
...
#ifdef _UNICODE
 #define BCG_TCHAR TCHAR
#else
 #define BCG_TCHAR UCHAR
#endif

/////////////////////////////////////////////////////////////////////////////
// CBCGMaskEdit class

IMPLEMENT_DYNAMIC(CBCGMaskEdit, CEdit)

BEGIN_MESSAGE_MAP(CBCGMaskEdit, CEdit)
	//{{AFX_MSG_MAP(CBCGMaskEdit)
	ON_WM_CHAR()
	ON_WM_KEYDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_CREATE()
...
void CBCGMaskEdit::DisableMask()
{
	m_strMask.Empty();
	m_strInputTemplate.Empty();
}

void CBCGMaskEdit::SetValidChars(LPCTSTR lpszValid)
{
	if (lpszValid != NULL)
	{
		m_strValid = lpszValid;
...
BOOL CBCGMaskEdit::IsMaskedChar(TCHAR chChar, TCHAR chMaskChar) const
{
	// ------------------------------
...
BOOL CBCGMaskEdit::IsValidChar(TCHAR chChar) const
{
...
void CBCGMaskEdit::SetWindowText(LPCTSTR lpszString)
{
...
int CBCGMaskEdit::GetWindowText(LPTSTR lpszStringBuf, int nMaxCount) const
{
...
int CBCGMaskEdit::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CBCGMaskEdit::OnCharDelete(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	ASSERT(m_strMask.IsEmpty() == m_strInputTemplate.IsEmpty());
	ASSERT(m_strMask.GetLength() == m_strInputTemplate.GetLength());
...
void CBCGMaskEdit::GetGroupBounds(int &nBegin, int &nEnd, int nStartPos, BOOL bForward)
{
	ASSERT(m_strMask.IsEmpty() == m_strInputTemplate.IsEmpty());
	ASSERT(m_strMask.GetLength() == m_strInputTemplate.GetLength());
