...
IMPLEMENT_DYNAMIC(CBCGPopupWindow, CWnd)
...
void CBCGPopupWndButton::OnFillBackground (CDC* pDC, const CRect& rectClient)
{
	CBCGVisualManager::GetInstance ()->
		OnErasePopupWindowButton (pDC, rectClient, this);
}
//*************************************************************************************
void CBCGPopupWndButton::OnDrawBorder (CDC* pDC, CRect& rectClient, UINT /*uiState*/)
{
	CBCGVisualManager::GetInstance ()->OnDrawPopupWindowButtonBorder (pDC, rectClient, this);
}
//*************************************************************************************
void CBCGPopupWndButton::OnDraw (CDC* pDC, const CRect& rect, UINT uiState)
{
	if (CBCGVisualManager::GetInstance ()->
		IsDefaultWinXPPopupButton (this))
	{
		return;
	}

	CBCGButton::OnDraw (pDC, rect, uiState);
}

/////////////////////////////////////////////////////////////////////////////
// CBCGPopupWindow

CBCGPopupWindow::CBCGPopupWindow()
{
	m_pWndOwner = NULL;
	m_bIsActive = FALSE;
	...
	m_AnimationType = CBCGPopupMenu::NO_ANIMATION;
	m_AnimationSpeed = 30;
	m_nAnimationAlpha = 0;
...
BEGIN_MESSAGE_MAP(CBCGPopupWindow, CWnd)
	//{{AFX_MSG_MAP(CBCGPopupWindow)
	ON_WM_TIMER()
	ON_WM_NCDESTROY()
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
...
/////////////////////////////////////////////////////////////////////////////
// CBCGPopupWindow message handlers

BOOL CBCGPopupWindow::Create(CWnd* pWndOwner, UINT uiDlgResID, HMENU hMenu, CPoint ptPos,
							  CRuntimeClass* pRTIDlgBar)
{
	ASSERT (pRTIDlgBar->IsDerivedFrom (RUNTIME_CLASS (CBCGPopupDlg)));

	m_hMenu = hMenu;
	m_pWndOwner = pWndOwner;
	m_uiDlgResID = uiDlgResID;

	m_pWndDlg = (CBCGPopupDlg*) pRTIDlgBar->CreateObject ();
	ASSERT_VALID (m_pWndDlg);

	return CommonCreate (ptPos);
}
//*******************************************************************************
BOOL CBCGPopupWindow::Create (CWnd* pWndOwner, CBCGPopupWndParams& params, HMENU hMenu, CPoint ptPos)
{
	m_hMenu = hMenu;
	m_pWndOwner = pWndOwner;

	m_pWndDlg = new CBCGPopupDlg;
	ASSERT_VALID (m_pWndDlg);
...
//*******************************************************************************
BOOL CBCGPopupWindow::CommonCreate (CPoint ptPos, CBCGPopupWndParams* pParams)
{
	m_ptLastPos = ptPos;
	...
	if (m_uiDlgResID != 0)
	{
		...
		dlgt.Load (MAKEINTRESOURCE (m_uiDlgResID));
	...
	if (m_hMenu != NULL)
	{
		...
		m_btnMenu.SetStdImage (CMenuImages::IdArowDownLarge);
		m_btnMenu.m_bDrawFocus = FALSE;
		m_btnMenu.m_nFlatStyle = CBCGButton::BUTTONSTYLE_FLAT;
...
//*******************************************************************************
void CBCGPopupWindow::OnTimer(UINT_PTR nIDEvent) 
{
	switch (nIDEvent)
	{
	case iAnimTimerId:
		if (!m_bAnimationIsDone)
		{
			...
			{
			case CBCGPopupMenu::UNFOLD:
				m_AnimSize.cx += nSteps * m_nAnimStepX;
				// no break intentionally

			case CBCGPopupMenu::SLIDE:
				m_AnimSize.cy += nSteps * m_nAnimStepY;
				break;

			case CBCGPopupMenu::FADE:
			...
			if (m_bFadeOutAnimation &&
				...
				(m_AnimationType == CBCGPopupMenu::FADE && m_iFadePercent <= 0)))
			{
				SendMessage (WM_CLOSE);
				return;
			}

			if ((m_AnimationType != CBCGPopupMenu::FADE && m_AnimSize.cy >= m_FinalSize.cy && m_AnimSize.cx >= m_FinalSize.cx) ||
				(m_AnimationType == CBCGPopupMenu::UNFOLD && m_AnimSize.cx >= m_FinalSize.cx) ||
				(m_AnimationType == CBCGPopupMenu::FADE && m_iFadePercent > 100) ||
				m_bIsActive)
			{
			...
			if (m_bFadeOutAnimation && m_AnimationType != CBCGPopupMenu::FADE)
			{
				CRect rectWnd;
...
//*******************************************************************************
void CBCGPopupWindow::OnNcDestroy() 
{
	CWnd::OnNcDestroy();
	delete this;
}
//*******************************************************************************
BOOL CBCGPopupWindow::OnEraseBkgnd(CDC* /*pDC*/)
{
	return TRUE;
}
//*******************************************************************************
void CBCGPopupWindow::OnPaint() 
{
...
//*******************************************************************************
CRect CBCGPopupWindow::GetCaptionRect ()
{
	int nCaptionHeight = GetCaptionHeight ();
...
//*******************************************************************************
BOOL CBCGPopupWindow::ProcessCommand (HWND hwnd)
{
	ASSERT_VALID (m_pWndDlg);
...
//*******************************************************************************
BOOL CBCGPopupWindow::OnCommand(WPARAM wParam, LPARAM lParam) 
{
...
//*******************************************************************************
void CBCGPopupWindow::OnDestroy() 
{
	if (m_pWndDlg != NULL)
	{
...
//*******************************************************************************
LRESULT CBCGPopupWindow::OnPrintClient(WPARAM wp, LPARAM lp)
{
	if (lp & PRF_CLIENT)
	{
...
//*******************************************************************************
void CBCGPopupWindow::OnDraw (CDC* pDC)
{
	ASSERT_VALID (pDC);
	...
	CBCGVisualManager::GetInstance ()->OnDrawPopupWindowBorder (pDC, rectClient);

	CRect rectCaption = GetCaptionRect ();

	COLORREF clrText = CBCGVisualManager::GetInstance ()->OnDrawPopupWindowCaption 
	...
	if (hIcon != NULL)
	{
		...
		rectText.left = rectImage.right + IMAGE_MARGIN;
	}
...
//*******************************************************************************
void CBCGPopupWindow::StartAnimation (BOOL bShow/* = TRUE*/)
{
	if (m_AnimationType == CBCGPopupMenu::NO_ANIMATION ||
		globalData.bIsRemoteSession ||
		(m_AnimationType == CBCGPopupMenu::FADE && globalData.m_nBitsPerPixel <= 8))
	{
		if (!bShow)
		{
			SendMessage (WM_CLOSE);
...
//*******************************************************************************
void CBCGPopupWindow::DrawAnimation (CDC* pPaintDC)
{
	CRect rectClient;
	GetClientRect (&rectClient);
	...
	if (m_bmpScreenDst.GetSafeHandle() == NULL)
	{
		...
		if (m_AnimationType == CBCGPopupMenu::FADE || globalData.m_nBitsPerPixel > 8)
		{
...
//*******************************************************************************
int CBCGPopupWindow::GetCaptionHeight ()
{
	if (m_bSmallCaption)
	{
...
//*******************************************************************************
void CBCGPopupWindow::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos)
{
	CWnd::OnWindowPosChanged(lpwndpos);
}
//*******************************************************************************
void CBCGPopupWindow::StartWindowMove ()
{
	m_bMoving = TRUE;
...
//*******************************************************************************
void CBCGPopupWindow::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_bMoving)
	{
		// ---------------------
...
//*******************************************************************************
void CBCGPopupWindow::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (m_bMoving)
	{
		ReleaseCapture ();
...
//*******************************************************************************
void CBCGPopupWindow::OnCancelMode() 
{
	CWnd::OnCancelMode();
...
//*******************************************************************************
void CBCGPopupWindow::OnLButtonDown(UINT /*nFlags*/, CPoint /*point*/) 
{
	StartWindowMove ();
}
